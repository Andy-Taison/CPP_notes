...
* Allows user-defined types (classes/structs) to define custom behavior for operators (`+`, `-`, `<<`, etc.)  
* Syntax: declare `operator<symbol>` as a function  
...
public:
    ...
    // Overload << operator as friend (non-member, needs access to private fields)
...
int main() {
    Point a(1, 2), b(3, 4);
    Point c = a + b;              // uses overloaded +
    std::cout << c << std::endl;  // uses overloaded <<
}
```
**Best Practices**  
* Prefer non-member overloads (especially for symmetric operators like `+`) unless operator must modify internal state  
* Keep overloaded operators intuitive (don’t give `+` the behavior of `-`)  
* For stream operators (`<<`, `>>`), implement as non-member (often [friend](#friend-functions))  
* Operators that don’t modify the object should be `const`  

Operators that **can** be overloaded: 
`+`, `-`, `*`, `/`, `%`, `==`, `!=`, `<`, `>`, `<<`, `>>`, `[]`, `()`, `=`, `->`, `++`, `--`  

Operators that **cannot** be overloaded: `.` (dot), `.*` (pointer-to-member dot), `::` (scope resolution), `sizeof`, `?:` (ternary conditional), `typeid`  
...
* For more control and reporting (e.g., test cases, setup/teardown), use third-party frameworks like **Catch2**, **Google Test (gtest)** or **Boost.Test**  
* Purpose: automatically verify that small units of code (functions/classes) behave as expected  


A unit test typically has:  
* Setup: prepare input/environment  
* Act: call the function under test  
* Assert: check the result matches expectation  
